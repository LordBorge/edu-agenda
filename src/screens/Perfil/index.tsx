...
import { generateScheduleSlots, isCompleteTime, ScheduleSlot } from '../../utils/time';
import { useAppTheme } from '../../theme';
import { TimeInput } from '../../components/TimeInput';

type SettingsForm = Omit<ScheduleSettings, 'id'>;
...
function buildSlots(form: SettingsForm): ScheduleSlot[] {
  return generateScheduleSlots({
    ...
    lunchDuration: form.lunch_duration,
...
export function PerfilScreen() {
  ...
  const updateField = (key: keyof SettingsForm, value: string | number) => {
    commitForm({ ...form, [key]: value } as SettingsForm);
  };

  const updateNumberField = (
    key: 'lesson_duration' | 'break_duration' | 'break_after_lesson' |
      'lunch_duration' | 'afternoon_break_duration' | 'afternoon_break_after_lesson',
    value: string
  ) => {
  ...
  const handleSave = async () => {
    const timeFields = [
      { label: 'horário de início do dia', value: form.start_time },
      { label: 'horário de término do dia', value: form.end_time },
      ...(form.period === 'integral' ? [{ label: 'horário de início do almoço', value: form.lunch_start }] : []),
    ];
    const invalidTime = timeFields.find(field => !isCompleteTime(field.value));

    if (invalidTime) {
      Alert.alert('Atenção', `Informe o ${invalidTime.label} no formato HH:MM.`);
      return;
    }

    await Promise.all([
      updateScheduleSettings(form),
      updateProfessionalProfile({
    ...
    const [persistedSettings, persistedProfile] = await Promise.all([
      getScheduleSettingsForPeriod(form.period),
      getProfessionalProfile(),
    ]);
  ...
  const handleThemeChange = async (themePreference: ThemePreference) => {
    ...
    await updateProfessionalProfile({
      ...
      onboarded: persistedProfile.onboarded || 1,
    });
  };

  const isIntegral = form.period === 'integral';
  const isAfternoon = form.period === 'tarde';
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        <View style={[styles.profileCard, { backgroundColor: colors.surface }]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.profileName, { color: colors.text }]}>{profileForm.name.trim() || 'Perfil profissional'}</Text>
        ...
        <TextInput
          ...
          placeholder="Ex: Inglês, Redação, Projeto de Vida"
          multiline
        />
        ...
        {isAfternoon ? (
          <>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>Intervalo da tarde</Text>
            <View style={styles.timeRow}>
              <View style={styles.timeBox}>
          ...
          <>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>{isIntegral ? 'Intervalo da manhã' : 'Intervalo'}</Text>
            <View style={styles.timeRow}>
              <View style={styles.timeBox}>
                <Text style={[styles.fieldLabel, fieldLabelTheme]}>Duração (min)</Text>
                <TextInput
          ...
          <>
            ...
            <View style={styles.timeRow}>
              ...
              <View style={styles.timeBox}>
                ...
                <TextInput
                  style={[styles.smallInput, inputTheme]}
                  value={String(form.lunch_duration)}
                  onChangeText={value => updateNumberField('lunch_duration', value)}
                  keyboardType="number-pad"
                />
              </View>
            </View>
